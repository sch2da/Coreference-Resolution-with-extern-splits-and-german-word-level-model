...
def get_document(doc_key, doc_lines, language, seg_len, tokenizer, input_format):
    ...
    if input_format != 'nested_list':
        for line in doc_lines:
            ...
            if len(row) == 0:
                if document_state.sentence_end:
...
def minimize_partition(partition, extension, args, tokenizer):
    ...
    with open(input_path, 'r') as input_file:
        for line in input_file.readlines():
            doc_key = ""
            if "dirndl" in args.input_suffix and line.startswith('#begin document'):
                splitted = line.split()
                doc_key = splitted[2].replace("(","").replace(")","").replace(";","")                
                part_id = int(splitted[-1][-3:].replace("(","").replace(")", "").replace(";",""))
                doc_key = doc_key + "_" + str(part_id)
