...
if __name__ == '__main__':
    ...
    start_peak_gpu = torch.cuda.max_memory_allocated(args.gpu)
    logger.info("Start Peak GPU value: " + str(start_peak_gpu))
    runner = Runner(args.config, args.gpu)
    model = runner.initialize_model()
    logger.info("Model size: " + str(torch.cuda.max_memory_allocated(args.gpu)))
    if args.model:
    ...
    with output_running_time():
        ...
        logger.info("\nEnd GPU value: " + str(end_peak_gpu))
        logger.info("Peak memory use: " + str(round(diff_peak_gpu / 1024 ** 3, 3)) + " GB")
        logger.info("Time without conll scorer: " + str(diff_time).split(".")[0])
