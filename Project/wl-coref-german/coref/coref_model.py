...
class CorefModel:  # pylint: disable=too-many-instance-attributes
    ...
    def evaluate(self,
        ...
        if self.epochs_trained == 1:
            if not os.path.exists(self.config.conll_log_dir):
                os.mkdir(self.config.conll_log_dir)
            basename = self.config.section + "_" + data_split
            old_files = [f for f in os.listdir(self.config.conll_log_dir) if basename in f]   
            for f in old_files:
                os.remove(self.config.conll_log_dir + "/" + f)
        ...
        for doc in pbar:
            ...
            if self.epochs_trained == self.config.train_epochs:
                with conll.open_(self.config, self.epochs_trained, data_split) as (gold_f, pred_f):
                    if word_level_conll:
