...
class PairwiseEncoder(torch.nn.Module):
    ...
    def __init__(self, config: Config):
        ...
        self.genre2int = {g: gi for gi, g in enumerate(["n", "bc", "bn", "mz", "nw",
                                                        "pt", "tc", "wb"])}
    ...
    def forward(self,  # type: ignore  # pylint: disable=arguments-differ  #35566 in pytorch
        ...
        speaker_map = torch.tensor(self._speaker_map(doc, self.config.with_speaker), device=self.device)
        ...
        genre = torch.tensor(self.genre2int[self.config.genre], device=self.device).expand_as(top_indices)
        genre = self.genre_emb(genre)

        return self.dropout(torch.cat((same_speaker, distance, genre), dim=2))

    @staticmethod
    def _speaker_map(doc: Doc, with_speaker: bool) -> List[int]:
        """
        Returns a tensor where i-th element is the speaker id of i-th word.
        """
        if with_speaker:
            doc["speaker"] = len(doc["speaker"]) * ["-"]
