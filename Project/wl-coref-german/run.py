...
if __name__ == "__main__":
    ...
    start_time = datetime.datetime.strftime(datetime.datetime.now(), "%Y.%m.%d_%H.%M")
    print("\n##### Start model training - " + start_time + " (UTC) #####", flush=True)
    start_peak_gpu = ""
    if "cuda" in CorefModel._load_config(args.config_file, args.experiment).device:
    ...
    if "cuda" in model.config.device:
        print("Model size: " + str(torch.cuda.max_memory_allocated()), flush=True)  
